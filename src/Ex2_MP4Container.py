...
def Ex2_main():
    ...
    os.system(
        "ffmpeg -ss 60 -i Resistencia_BM19_cropped.mp4 -vcodec copy -acodec copy Resistencia_MP4_Container.mp4")  # Recorta los primeros 60 segundos del video.
    time.sleep(2)
    ...
    while aux == True:
        ...
        if res == '1':
            ...
            os.system(
                "ffmpeg -i Resistencia_MP4_Container.mp4 -i Resistencia_audio.mp3 -map 0:v -c:v copy -map 0:a -c:a copy -map 1:a -c:a copy  Resistencia_MP4_Container_MP3.mp4")
            ...
            os.system(
                "ffmpeg -i Resistencia_MP4_Container_MP3.mp4 -i Resistencia_BM19_cropped.aac -map 0:v -c:v copy -map 0:a -c:a copy -map 1:a -c:a copy  Resistencia_MP4_Container_AAC.mp4")
            print(f"¡Convertido correctamente!")
            time.sleep(1)
            count += 1
            continue
            ...
            while i < 10:
                ...
                time.sleep(0.2)
            os.system(
                "ffmpeg -i Resistencia_BM19_cropped.mp4 -vn -c:a ac3 -b:a 384k -ar 48000 -ac 6 Resistencia_BM19_cropped.ac3")
            os.system(
                "ffmpeg -i Resistencia_MP4_Container_MP3.mp4 -i Resistencia_BM19_cropped.ac3 -map 0:v -c:v copy -map 0:a -c:a copy -map 1:a -c:a copy  Resistencia_MP4_Container_AC3.mp4")
            print(f"¡Convertido correctamente!")
            time.sleep(1)
            count += 1
            continue
