...
def Ex3_main():
    ...
    os.system(
        "ffprobe -hide_banner -v error -show_entries stream=index,codec_type,codec_name -of default=noprint_wrappers=1 -print_format json " + str(
        ...
        for stream in data['streams']:  # FILTRAMOS POR STREAMS:
            if stream['codec_type'] == 'video':  # SELECCIONAMOS LOS STREAMS DE VIDEO
                ...
                for bCast_standard in bs_list['broadcasting_standards']:  # RECORREMOS LA LISTA DE BROADCAST STANDARDS.
                    bCast_video = bCast_standard[
                        'video_codecs']  # Guardamos los codecs de video del broadcast en esta iteración.
                    ...
                    for vcodec in bCast_video:  # Recorremos cada codec de video del estándar y comparamos:
                        if vcodec == data_video_codec:
                            video_compatibilities.append(
                                bCast_standard['name'])  # Guardamos aquellos estándares compatibles en video.
            elif stream['codec_type'] == 'audio':  # SELECCIONAMOS LOS STREAMS DE AUDIO
                data_audio_codec = stream['codec_name']
                for bCast_standard in bs_list['broadcasting_standards']:  # RECORREMOS LA LISTA DE BROADCAST STANDARDS.
                    bCast_audio = bCast_standard[
                        'audio_codecs']  # Guardamos los codecs de audio del broadcast en esta iteración.
                    ...
                    for acodec in bCast_audio:  # Recorremos cada codec de audio del estándar y comparamos:
                        if acodec == data_audio_codec:
                            audio_compatibilities.append(
                                bCast_standard['name'])  # Guardamos aquellos estándares compatibles en audio.
            final_compatibilities = set(video_compatibilities).intersection(
                audio_compatibilities)  # Seleccionamos aquellos estándares compatibles tanto en vídeo como en audio.
        if len(final_compatibilities) == 0:
            print(
                f"ERROR: Lo sentimos. No hemos podido detectar ningún estándarde broadcasting compatible con este formato.")
        else:
            print(f"Los estándares de broadcasting compatibles con este formato multimedia son: " + str(
                final_compatibilities) + ".")
